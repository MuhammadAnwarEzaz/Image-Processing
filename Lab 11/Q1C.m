...
%Remove the border merging dots from image
II = (I & ~I_BORDER);
Area = 225;
IVIS = false(R,C);
I_OVERLAP = false(R,C);
...
%Remove border merging dots and remove overlapping dots to get only single dots 
I_SINGLE = I & ~I_BORDER & ~I_OVERLAP;
imshow(I_SINGLE);
title('Non Overlapping Particles');
imwrite(I_SINGLE,'Output_Q1C.jpg');